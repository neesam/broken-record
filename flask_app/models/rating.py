...
class Rating:
    ...
    def save(cls, data):
        ...
        query = 'INSERT INTO ratings (stars, artist, album, cover, year, artist_link, album_link, release_date, tracks, album_id, created_at, updated_at, bunk, artist_id, user_id, content) VALUES (%(stars)s, %(artist)s, %(album)s, %(cover)s,  %(year)s, %(artist_link)s, %(album_link)s, %(release_date)s, %(tracks)s, %(album_id)s, NOW(), NOW(), %(bunk)s, %(artist_id)s, %(user_id)s, %(content)s)'
    ...
    def update_rating(cls, data):
        ...
        query = 'UPDATE ratings SET stars=%(stars)s, artist=%(artist)s, album=%(album)s, cover=%(cover)s, year=%(year)s, artist_link=%(artist_link)s, album_link=%(album_link)s, release_date=%(release_date)s, tracks=%(tracks)s, album_id=%(album_id)s, artist_id=%(artist_id)s, bunk=%(bunk)s, updated_at=NOW(), content=%(content)s WHERE ratings.id = %(id)s'
